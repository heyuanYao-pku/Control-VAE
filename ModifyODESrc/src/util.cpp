...
void dxStepBody (dxBody *b, dReal h)
{
  ...
  // printf("dxstepbody pos = %lf, %lf, %lf, vel = %lf, %lf, %lf, ", b->posr.pos[0], b->posr.pos[1], b->posr.pos[2], b->lvel[0], b->lvel[1], b->lvel[2]);
  for (unsigned int j=0; j<3; j++) b->posr.pos[j] += h * b->lvel[j];
  // printf("after pos = %lf, %lf, %lf\n", b->posr.pos[0], b->posr.pos[1], b->posr.pos[2]);
...
// Add by Zhenhua Song
void dxProcessIslandsWithInfo(
    dxWorld* world, 
    const dxWorldProcessIslandsInfo& islandsinfo,
    dReal stepsize,
    dstepper_with_info_fn_t stepper, 
    WorldStepFeatureInfoPtr feature_info)
{
    const unsigned int sizeelements = 2;
    ...
    for (unsigned int const* sizescurr = islandsizes; sizescurr != sizesend; sizescurr += sizeelements) {
        ...
        BEGIN_STATE_SAVE(stepperarena, stepperstate) {
            // now do something with body and joint lists
            stepper(stepperarena, world, bodystart, bcount, jointstart, jcount, stepsize, feature_info);
        } END_STATE_SAVE(stepperarena, stepperstate);
