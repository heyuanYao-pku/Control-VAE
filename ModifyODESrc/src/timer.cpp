...
static inline void getClockCount (unsigned long cc[2])
{
  ...
  auto tt = t.time_since_epoch().count();
  cc[1] = static_cast<unsigned long>(tt / 1000000);
  cc[0] = static_cast<unsigned long>(tt - ( cc[1] * 1000000 ));
}
...
void dTimerReport (FILE *fout, int average)
{
  ...
  for (i=0; i<num; i++) {
    ...
    fprintf (fout,"%-*s %10.5fms %7.3f%%",(int)maxl,event[i].description,
	     t*ccunit * 1000.0, p);
    if (average && i < (num-1)) {
      fprintf (fout,"  (avg %10.5fms %7.3f%%)",
