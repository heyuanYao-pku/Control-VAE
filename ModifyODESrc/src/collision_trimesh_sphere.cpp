...
int dCollideSTL(dxGeom* g1, dxGeom* SphereGeom, int Flags, dContactGeom* Contacts, int Stride){
	...
	if (TriMesh->doSphereTC) {
        ...
        if (TriMesh->isSingleTriangle)
            Collider.Collide(*sphereTC, Sphere, TriMesh->Data->BVTree, null, null);
        else
        ...
        // for single-triangle mesh
        if (TriMesh->isSingleTriangle)
            Collider.Collide(pccColliderCache->defaultSphereCache, Sphere, TriMesh->Data->BVTree, null, null);
        else
	...
	if (TriCount != 0){
		...
		for (int i = 0; i < TriCount; i++){
            ...
            if (TriMesh->ColNumCallback)
                TriMesh->ColNumCallback(TriMesh, SphereGeom, TriIndex, 1);
