    ...
    while (true) {
      ...
      // Switch to growing array forward
      {
        ...
        while (true) {	// jicurr=dest, _jcurr=src
          ...
          // changed by heyuan yao, implicit damping will add a constraint
          dIASSERT (jicurr->info.m >= 0 && jicurr->info.m <= 7 && jicurr->info.nub >= 0 && jicurr->info.nub <= jicurr->info.m);
          if (jicurr->info.m > 0) {
