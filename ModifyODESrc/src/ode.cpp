...
// Add By Zhenhua Song
void dBodySetRotAndQuatNoNorm(dBodyID b, const dMatrix3 R, const dQuaternion q) {
    dAASSERT(R && q);
    memcpy(b->posr.R, R, sizeof(dMatrix3));
    memcpy(b->q, q, sizeof(dQuaternion));
...
void dBodyCopyRotation (dBodyID b, dMatrix3 R) // Modify by Zhenhua Song
{
	dAASSERT (b);
	const dReal* src = b->posr.R;
    memcpy(R, src, sizeof(dReal) * 12);
}
...
int dBodyGetNumGeoms(dBodyID b)
{
    ...
    for (; g != NULL; g = dGeomGetBodyNext(g)) {
        cnt++;
    }
    return cnt;
}
...
void dBodyGetInertia(dBodyID b, dReal* out)
{
    ...
    dMultiply0_333(out, b->posr.R, tmp); // R * I * R^T
}
...
void dBodyGetInertiaInv(dBodyID b, dReal* out)
{
    ...
    dMultiply0_333(out, b->posr.R, tmp); // R * invI * R^T
}
...
// Add by Zhenhua Song
dxJoint* dJointCreateEmptyBall(dWorldID w, dJointGroupID group)
{
    dAASSERT(w);
    return createJoint<dxJointEmptyBall>(w, group);
}
...
dxJoint * dJointCreateContact (dWorldID w, dJointGroupID group, const dContact *c)
{
    dAASSERT (w && c);
...
// Add by Zhenhua Song. it is almost same as dJointCreateContact
dxJoint* dJointCreateContactMaxForce(dWorldID w, dJointGroupID group, const dContact* c)
{
    dAASSERT(w && c);
    dxJointContact* j = (dxJointContactMaxForce*)createJoint<dxJointContactMaxForce>(w, group);
    j->contact = *c;
    return j;
}
...
int dJointInGroup(dJointGroupID group, dJointID id)
{
    dAASSERT(group);
    int i;
...
void dJointGroupDestroy (dJointGroupID group) // This will deconstruct all joints in joint group
{
    dAASSERT (group);
...
// Add by Heyuan Yao
void dJointEnableImplicitDamping(dxJoint *joint)
{
  // dAASSERT(joint);
  joint->useImplicitDamping = true;
}

void dJointDisableImplicitDamping(dxJoint* joint)
{
  dAASSERT (joint);
  joint->useImplicitDamping = false;
}
...
// Add by Zhenhua Song
ODE_API void dJointSetKd_arr(dxJoint* joint, const dReal * kd) {
    dAASSERT(joint);

    joint->aveldamping[0] = kd[0];
    joint->aveldamping[1] = kd[1];
    joint->aveldamping[2] = kd[2];
}

// Add by Zhenhua Song
ODE_API void dJointSetKd(dxJoint * joint, dReal kdx, dReal kdy, dReal kdz) {
    dAASSERT(joint);

    joint->aveldamping[0] = kdx;
    joint->aveldamping[1] = kdy;
    joint->aveldamping[2] = kdz;
}

// Add by Zhenhua Song
ODE_API const dReal * dJointGetKd(dxJoint* joint) {
    dAASSERT(joint);

    return joint->aveldamping;
}
...
extern "C" int dWorldStep (dWorldID w, dReal stepsize)
{
  dUASSERT (w,"bad world argument");
...
// Add by Zhenhua Song
extern "C" int dWorldStepWithInfo(dWorldID w, dReal stepsize, WorldStepFeatureInfoPtr feature_info)
{
    dUASSERT(w, "bad world argument");
    dUASSERT(stepsize > 0, "stepsize must be > 0");
    ...
    if (dxReallocateWorldProcessContext(w, islandsinfo, stepsize, &dxEstimateStepMemoryRequirements))
    {
        //dxProcessIslandsWithInfo(w, islandsinfo, stepsize, &dInternalStepIslandWithInfo, feature_info);
        dxProcessIslandsWithInfo(w, islandsinfo, stepsize, dInternalStepIslandWithInfo, feature_info);
        result = true;
    }
...
// Add by Zhenhua Song
extern "C" int dWorldResortJoint(dWorldID w, dReal stepsize, WorldStepFeatureInfoPtr feature_info)
{
    dUASSERT(w, "bad world argument");
    dUASSERT(stepsize > 0, "stepsize must be > 0");
    ...
    {
        //dxProcessIslandsWithInfo(w, islandsinfo, stepsize, &dInternalStepIslandWithInfo, feature_info);
        dxProcessIslandsWithInfo(w, islandsinfo, stepsize, dInternalStepIslandResortJoint, feature_info);
        result = true;
    }
...
// Add by Zhenhua Song
dJointID dWorldGetFirstJoint(dWorldID w) 
{
    dAASSERT(w);
    return w->firstjoint;
}
...
int dWorldGetNumBallAndHingeJoints(dWorldID w)
{
    ...
    while (joint != NULL) {
        ...
        if (joint_type == dJointTypeBall || joint_type == dJointTypeHinge) {
            cnt++;
        }
        joint = (dxJoint*)(joint->next);
    }
    return cnt;
}
