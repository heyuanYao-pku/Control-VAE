...
void dMassSetTrimesh( dMass *m, dReal density, dGeomID g )
{
  ...
  Rz = T1[2] / T0;
	...
	m->_I(0,0) -= m->mass * (Ry*Ry + Rz*Rz);
	m->_I(1,1) -= m->mass * (Rx*Rx + Rz*Rz);
	m->_I(2,2) -= m->mass * (Ry*Ry + Rx*Rx);
	m->_I(0,1) += m->mass * Rx*Ry;
	m->_I(1,0) += m->mass * Rx*Ry;
	m->_I(2,1) += m->mass * Rz*Ry;
	m->_I(1,2) += m->mass * Rz*Ry;
	m->_I(2,0) += m->mass * Rx*Rz;
	m->_I(0,2) += m->mass * Rx*Rz;
