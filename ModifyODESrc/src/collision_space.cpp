...
void dxHashSpace::collide (void *data, dNearCallback *callback)
{
  ...
  /////////////////////
  // Added by Libin
  // sometimes n is very large... we have to alloc this array on stack
  bool flagVeryLargeN = n > 1000;
  int tested_rowsize = (n+7) >> 3;	// number of bytes needed for n bits
  unsigned char *tested = flagVeryLargeN ? (unsigned char *)malloc(n * tested_rowsize) : (unsigned char *) ALLOCA (n * tested_rowsize);
  ...
  Node **table = flagVeryLargeN ? (Node **) malloc (sizeof(Node*) * sz) : (Node **) ALLOCA (sizeof(Node*) * sz);
  for (i=0; i<sz; i++) table[i] = 0;

  // add each AABB to the hash table (may need to add it to up to 8 cells)
  Node *nodeBuf = flagVeryLargeN ? (Node *) malloc (sizeof (Node) * sz) : NULL;
  Node *nodeBufNext = nodeBuf;
  if (flagVeryLargeN)
  {
        ...
        for (int xi = dbounds[0]; xi <= dbounds[1]; xi++) {
          for (int yi = dbounds[2]; yi <= dbounds[3]; yi++) {
	        for (int zi = dbounds[4]; zi <= dbounds[5]; zi++) {
	          ...
	          Node *node = nodeBufNext++;//(Node*) malloc (sizeof (Node));
	          node->x = xi;
	          node->y = yi;
	          node->z = zi;
	          node->aabb = aabb;
...
void dSpaceAdd (dxSpace *space, dxGeom *g)
{
  ...
  // std::cout<<"?"<<g->parent_space<<std::endl;
  space->add (g);
}
...
// Add by Zhenhua Song
dGeomID dSpaceGetFirstGeom(dxSpace* space) 
{
    dAASSERT(space);
    dUASSERT(dGeomIsSpace(space), "argument not a space");
    return space->first;
...
// Add by Zhenhua Song
#include <algorithm>
#define DebugResortGeoms 0
void dSpaceResortGeoms(dSpaceID space)
{
    // NOTE: do not support sub space
    dAASSERT(space);
    dUASSERT(dGeomIsSpace(space), "argument not a space");
    // pop out all of geometries..alloc memory on the heap...
    int num_geom = space->getNumGeoms();
#if DebugResortGeoms
    printf("num geom %d in space\n", num_geom);
#endif
    dGeomID* geom_arr = new dGeomID[num_geom];
    memset(geom_arr, 0, sizeof(dGeomID) * num_geom);
    dGeomID g = space->first;
    bool all_not_set = true;
    for (int idx = 0; idx < num_geom; idx++)
    {
#if DebugResortGeoms
        printf("Geom id %lld\n", g);
        printf("Geom type %d %d, isSpace(%d), character_id = %d, index = %d\n", idx, g->type, dGeomIsSpace(g), g->character_id, g->geom_index); // for debug
#endif
        geom_arr[idx] = g;
        all_not_set &= g->character_id == -1;
        g = g->next;
    }
    if (all_not_set)
    {
        if (num_geom > 0)
        {
          printf("geometry character id are not set. return..\n"); // for debug
        }
        delete[] geom_arr;
        return;
    }

    for (int idx = 0; idx < num_geom; idx++) // remove all of geoms in space
    {
        space->remove(geom_arr[idx]);
    }
#if DebugResortGeoms
    printf("end remove: space count = %d\n", space->getNumGeoms());
#endif
    std::sort(geom_arr, geom_arr + num_geom, [](dGeomID a, dGeomID b) {
        ...
        {
            return a->geom_index < b->geom_index;
        }
    });
#if DebugResortGeoms
    printf("end sort\n");
#endif
    // for (int idx = num_geom - 1; idx >= 0; idx--) // add all of geoms to space..
    for(int idx = 0; idx < num_geom; idx++)
    {
        space->add(geom_arr[idx]);
    }
#if DebugResortGeoms
    for (g = space->first; g != NULL; g = g->next)
    {
        printf("geom %lld, character id = %d, index = %d\n", g, g->character_id, g->geom_index);
    }
#endif
    delete[] geom_arr;
}
...
// Add by Zhenhua Song
void * dSpaceGetData(dxSpace * space)
{
  dAASSERT (space);
  dUASSERT (dGeomIsSpace(space),"argument not a space");
  return space->data;
...
// Add by Zhenhua Song
void dSpaceSetData(dxSpace * space, void * data)
{
  dAASSERT (space);
  dUASSERT (dGeomIsSpace(space),"argument not a space");
  space->data = data;
