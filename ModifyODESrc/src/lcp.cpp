...
static dtype * dTypeMemAlloc(dxWorldProcessMemArena* memarena, size_t count)
{
    if (memarena != NULL)
    {
        return memarena->AllocateArray<dtype>(count);
    }
    else
    {
        return new dtype[count];
    }
}

// Add by Zhenhua Song
template<typename dtype>
static void dTypeMemDelete(dxWorldProcessMemArena* memarena, dtype * data)
{
    if (memarena != NULL)
    {
        return;
...
// This will destroy A, b
// memory part is modified by Zhenhua Song
...
# endif
  ...
  // if all the variables are unbounded then we can just factor, solve Ax=b using LDLT,
  // and return
  if (nub >= n) {
    dReal* d = dTypeMemAlloc<dReal>(memarena, n); // memarena->AllocateArray<dReal> (n);
    dSetZero (d, n);
    ...
    dTypeMemDelete<dReal>(memarena, d);
    return;
  }

  const int nskip = dPAD(n);
  dReal* L = dTypeMemAlloc<dReal>(memarena, n * nskip);// memarena->AllocateArray<dReal> (n*nskip);
  dReal* d = dTypeMemAlloc<dReal>(memarena, n); // memarena->AllocateArray<dReal> (n);
  dReal* w = outer_w ? outer_w : dTypeMemAlloc<dReal>(memarena, n); // memarena->AllocateArray<dReal> (n);
  dReal* delta_w = dTypeMemAlloc<dReal>(memarena, n); // memarena->AllocateArray<dReal> (n);
  dReal* delta_x = dTypeMemAlloc<dReal>(memarena, n); // memarena->AllocateArray<dReal> (n);
  dReal* Dell = dTypeMemAlloc<dReal>(memarena, n); // memarena->AllocateArray<dReal> (n);
  dReal* ell = dTypeMemAlloc<dReal>(memarena, n); // memarena->AllocateArray<dReal> (n);
#ifdef ROWPTRS
  dReal** Arows = dTypeMemAlloc<dReal*>(memarena, n);  // memarena->AllocateArray<dReal*>(n);
#else
  dReal **Arows = NULL;
#endif
  int* p = dTypeMemAlloc<int>(memarena, n); //  memarena->AllocateArray<int>(n);
  int* C = dTypeMemAlloc<int>(memarena, n); //  memarena->AllocateArray<int>(n);

  // for i in N, state[i] is 0 if x(i)==lo(i) or 1 if x(i)==hi(i)
  bool* state = dTypeMemAlloc<bool>(memarena, n); // memarena->AllocateArray<bool> (n);
    ...
    if (lo[i]==0 && w[i] >= 0) {
      ...
      for (;;) {
        ...
        case 5:		// keep going
          ...
          lcp.transfer_i_from_C_to_N (si, memarena == NULL ? NULL : memarena->PeekBufferRemainder()); // Modify by Zhenhua Song
          break;
        case 6:		// keep going
          x[si] = hi[si];
          state[si] = true;
  ...
  dTypeMemDelete<dReal>(memarena, L);
  dTypeMemDelete<dReal>(memarena, d);
  if (outer_w == NULL)
  {
      dTypeMemDelete<dReal>(memarena, w);
  }
  dTypeMemDelete<dReal>(memarena, delta_w);
  dTypeMemDelete<dReal>(memarena, delta_x);
  dTypeMemDelete<dReal>(memarena, Dell);
  dTypeMemDelete<dReal>(memarena, ell);
#ifdef ROWPTRS
  dTypeMemDelete<dReal*>(memarena, Arows);
#endif
  dTypeMemDelete<int>(memarena, p);
  dTypeMemDelete<int>(memarena, C);
  dTypeMemDelete<bool>(memarena, state);
}

static size_t EstimateTestSolveLCPMemoryReq(int n);
// Add by Zhenhua Song
extern "C" void dSolveLCPWrapper(int n, dReal* A, dReal* x, dReal* b,
    dReal* outer_w, int nub, dReal* lo, dReal* hi, int* findex)
{
    ...
    if (arena == NULL) 
    {
        dDebug(0, "mem arena is NULL");
        return;
    }
    dSolveLCP(arena, n, A, x, b, outer_w, nub, lo, hi, findex);
