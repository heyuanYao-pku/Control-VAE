...
dxJoint * dJointCreateContact3(dWorldID w, dJointGroupID group,
    const dContact *c, dReal timestep)
{
    dAASSERT(w && c);
    dxJoint *j0;
    if (group) {
        j0 = (dxJointContact3*)group->stack.alloc(sizeof(dxJointContact3));
        group->num++;
    }
    else
        j0 = (dxJointContact3*)dAlloc(sizeof(dxJointContact3));

    dxJointContact3 *j = new(j0)dxJointContact3(w);
    if (group)
...
dxJointContact3::dxJointContact3(dxWorld *w)
    : dxJoint(w)
{
}
...
void
dxJointContact3::getSureMaxInfo(SureMaxInfo* info)
{
...
void
dxJointContact3::getInfo1(dxJoint::Info1 *info)
{
...
void
dxJointContact3::getInfo2(dxJoint::Info2 *info)
{
    int s = info->rowskip;
