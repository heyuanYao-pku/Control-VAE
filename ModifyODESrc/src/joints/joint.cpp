        ...
        dObject( w )
{
    ...
    // added by Libin Liu
    // for damped step
    dSetZero(aveldamping, 4);
    isAnisotropicDamping = false;
    useImplicitDamping = false;
    dampingRefBody = 0;
}
              ...
              dVector3 anchor1, dVector3 anchor2 )
{
    ...
    info->J1l[s+1] = 1;
    info->J1l[2*s+2] = 1;
    dMultiply0_331( a1, joint->node[0].body->posr.R, anchor1 ); // R * anchor1
    dSetCrossMatrixMinus( info->J1a, a1, s ); // cross vector -> - cross matrix
    if ( joint->node[1].body )
    {
        info->J2l[0] = -1;
        info->J2l[s+1] = -1;
        info->J2l[2*s+2] = -1;
        dMultiply0_331( a2, joint->node[1].body->posr.R, anchor2 ); // R * anchor2
        dSetCrossMatrixPlus( info->J2a, a2, s ); // cross vector -> cross matrix
    }
...
void setAxes( dxJoint *j, dReal x, dReal y, dReal z, dVector3 axis1, dVector3 axis2 )
{
    if ( j->node[0].body )
    {
        dReal q[4];
...
dReal getHingeAngle(dxBody *body1, dxBody *body2, dVector3 axis, dQuaternion q_initial )
{
    ...
    {
        dQuaternion qq;
        dQMultiply1( qq, body1->q, body2->q ); // inv(q1) * q2
        dQMultiply2( qrel, qq, q_initial ); // inv(q1) * q2 * inv(q_initial)
    }
    else
    {
...
dReal dxJointLimitMotor::get( int num )
{
    ...
    case dParamVelMax: // add by Libin Liu
        return vel_max;
    default: // Default case modify by Zhenhua Song
        dDebug(0, "Parameter type not supported.\n");
        return 0;
    }
}

int dxJointLimitMotor::testRotationalLimit(dReal angle, dReal avel)
{
    int check = testRotationalLimit(angle);
    if (avel <= -vel_max)
    {
        vel_limit_err = avel + vel_max;
        vel_limit = 1;

        check = 1;
    }
    else if (avel >= vel_max)
    {
        vel_limit_err = avel - vel_max;
        vel_limit = 2;
...
/**
 * @brief add hinge/amotor (and etc) limits
 *
 * @param joint hinge/amotor (and etc) joint
 * @param info info2 for get jacobian
 * @param row row for add limit to jacobian. for hinge joint, it's 5.
 * @param ax1 for hinge joint, it's global hinge axis wrt body1
 * @param rotational true if joint is rotational, false if joint is translational
 * @return int
 */
int dxJointLimitMotor::addLimot(dxJoint *joint, dxJoint::Info2 *info, int row,
                                const dVector3 ax1, int rotational )
{
    ...
    int powered = fmax > 0;
    if ( powered || limit || vel_max != dInfinity)
    {
        ...
        // Comment by Zhenhua Song: This will not be executed if dParamVelMax is not set. 
        // So, this part can be ignored. 
        // TODO: calc m and nub in getInfo1.
        /* if (!powered && !limit) // Add by Libin Liu. 
        {
            //info->c[row] = -vel_limit_err;
            info->cfm[row] = stop_cfm;
            info->cfm[row + 1] = stop_cfm;
            ...
            info->c[row + 1] = vel_max;
            info->lo[row + 1] = -dInfinity;
            info->hi[row + 1] = 0;
        }*/
        return 1;
    }
...
/////////////////////////
// added by libin
// only rotational joint limit is support, ignore powored flag
////////////////////////
int dxJointLimitMotor::addLimotDot(dxJoint *joint, dxJoint::Info3 *info, int row,
    const dVector3 ax1, int rotational)
{
    int srow = row * info->rowskip;
    ...
    if (limit)
    {
        dReal *J1 = rotational ? info->Jd1a : info->Jd1l;
        dReal *J2 = rotational ? info->Jd2a : info->Jd2l;

        dCalcVectorCross3(J1, joint->node[0].body->avel, ax1);
        if (joint->node[1].body)
        {
            dCalcVectorCross3(J1, joint->node[1].body->avel, ax1);
            J2[srow + 0] = -J2[srow + 0];
            J2[srow + 1] = -J2[srow + 1];
            J2[srow + 2] = -J2[srow + 2];
        }
