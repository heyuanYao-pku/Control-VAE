...
struct dxJoint : public dObject
{
    ...
    //////////////////
    // add by libin
    // jacobian acceleration
    //////////////////
    struct Info3
    {
        dReal fps; // frames per second (1/stepsize),

        // for the first and second body, pointers to two (linear and angular)
        // n*3 jacobian sub matrices, stored by rows. these matrices will have
        // been initialized to 0 on entry. if the second body is zero then the
        // J2xx pointers may be 0.
        dReal *Jd1l, *Jd1a, *Jd2l, *Jd2a;
    ...
    ////////////////////////////////////////////////////////////
    // added by Libin Liu
    // anguler speed damping support
    dVector3 aveldamping; // kd
    bool isAnisotropicDamping;
    dxBody *dampingRefBody; // joint's parent. on local frame.
    
    // added by Heyuan Yao
    bool useImplicitDamping;
    ////////////////////////////////////////////////////////////
    ...
    ////////////////////////////
    // added by libin
    // get Jdot for bounded constraints
    virtual void getInfo3(Info3* info) { /*do nothing by default*/ }
    /////////////////////////////
...
struct dxJointGroup : public dBase
{
    ...
    dObStack stack; // a stack of (possibly differently sized) dxJoint objects.
};
...
struct dxJointLimitMotor
{
    ...
    ///////////////////////
    // add by Libin
    // currently the vel_max only applied when the joint 
    // is NOT powered and 
    // is NOT at the joint limit
    dReal vel_max;
    int vel_limit;
    dReal vel_limit_err;

    int testRotationalLimit(dReal angle, dReal avel);
    int addLimotDot(dxJoint *joint, dxJoint::Info3 *info, int row,
        const dVector3 ax1, int rotational);
};
