...
dxJointBall::getSureMaxInfo( SureMaxInfo* info )
{
    ...
    // add by Heyuan Yao
    if(useImplicitDamping) info->max_m = 6;
}
...
dxJointBall::getInfo2( dxJoint::Info2 *info )
{
    ...
    if(useImplicitDamping){
        if(!isAnisotropicDamping){
            dReal all_cfm = dRecip(aveldamping[0]);
            int s = info->rowskip;
            for(int i=0;i<3;i++){
                info->J1a[(i+3)*s+i] = 1;
                info->J2a[(i+3)*s+i] = -1;
                info->cfm[i+3] = all_cfm;
            }
        }
        else{
           dUASSERT(0, "not implemented error");
        }
...
void dxJointBall::set( int num, dReal value )
{
    ...
    case dParamERP:
        erp = value;
        break;
    // default case add by Zhenhua Song
    default:
        dDebug(0, "Type num = %d not supported. Only support dParamCFM = %d, dParamERP = %d\n", num, dParamCFM, dParamERP);
    }
}
...
dReal dxJointBall::get( int num )
{
    ...
    case dParamERP:
        return erp;
    default: // default case add by Zhenhua Song 
        dDebug(0, "Type num = %d not supported. Only support dParamCFM = %d, dParamERP = %d\n", num, dParamCFM, dParamERP);
        return 0;
    }
}
