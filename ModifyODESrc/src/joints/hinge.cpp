        ...
        dxJoint( w )
{
    ...
    erp = world->global_erp; // Add by Zhenhua Song
...
{
    info->max_m = 6;
    if (limot.vel_max != dInfinity)
        info->max_m = 8;
    if(useImplicitDamping) info->max_m++;
}
...
dxJointHinge::getInfo1( dxJoint::Info1 *info )
{
    ...
    // see if joint is powered. Comment by Zhenhua Song: The initial value of fmax is 0, so if we don't use powered hinge joint, we can ignore it.
    if ( limot.fmax > 0 )
    ...
    // Add by Heyuan Yao
    if (useImplicitDamping) { info->m++; info->nub++; }
    // Add by Libin Liu. Comment by Zhenhua Song: initial value of vel_max is dInfinty. If we don't set this value, this will not be considered.
    //if (info->m == 5 && limot.vel_max != dInfinity)
    //{
    //    info->m = 7;
    //}
}
...
dxJointHinge::getInfo2( dxJoint::Info2 *info )
{
    ...
    // printf("fps, %lf, erp %lf\n", info->fps, info->erp);
    info->c[3] = k * dCalcVectorDot3( b, p );
    info->c[4] = k * dCalcVectorDot3( b, q );
    if(useImplicitDamping) {
        /// calculate axis of hinge
        int s = info->rowskip;
        dCopyVector3(info->J1a+5*s, ax1);
        dCopyNegatedVector3(info->J2a+5*s, ax1);
        info->cfm[5] = dRecip(aveldamping[0]);
        // if the hinge is powered, or has joint limits, add in the stuff
        limot.addLimot( this, info, 6, ax1, 1 ); // ax1: the global hinge axis wrt body1
        return; // return immediately
    }

    // if the hinge is powered, or has joint limits, add in the stuff
    limot.addLimot( this, info, 5, ax1, 1 ); // ax1: the global hinge axis wrt body1
}
...
// Add by Zhenhua Song
const dReal * dJointGetHingeAnchor1Raw(dJointID j)
{
    // should consider dJOINT_REVERSE
    dxJointHinge* joint = (dxJointHinge*)j;
    dUASSERT(joint, "bad joint argument");
    checktype(joint, Hinge);
    if (joint->flags & dJOINT_REVERSE)
...
// Add by Zhenhua Song
const dReal* dJointGetHingeAnchor2Raw(dJointID j)
{
    // should consider dJOINT_REVERSE
    dxJointHinge* joint = (dxJointHinge*)j;
    dUASSERT(joint, "bad joint argument");
...
void dJointGetHingeAxis1(dJointID j, dVector3 result)
{
    ...
    if (joint->node[0].body)
    {
        dMultiply0_331(result, j->node[0].body->posr.R, joint->axis1);
    }
    else
    {
        dDebug(0, "Body 1 not exist.");
    }
}
...
void dJointGetHingeAxis2(dJointID j, dVector3 result)
{
    ...
    if (joint->node[1].body)
    {
        dMultiply0_331(result, j->node[1].body->posr.R, joint->axis2);
    }
    else
    {
        dDebug(0, "Body 2 not exist.");
    }
}
...
void dJointSetHingeParam( dJointID j, int parameter, dReal value ) // Modify by Zhenhua Song
{
    dxJointHinge* joint = ( dxJointHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    checktype( joint, Hinge );
...
dReal dJointGetHingeParam( dJointID j, int parameter ) // Modify by Zhenhua Song
{
    dxJointHinge* joint = ( dxJointHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    checktype( joint, Hinge );
...
dxJointHinge::computeInitialRelativeRotation()
{
    if ( node[0].body )
    {
        if ( node[1].body )
        {
            dQMultiply1( qrel, node[0].body->q, node[1].body->q ); // inv(q0) * q1
        }
        else
        {
