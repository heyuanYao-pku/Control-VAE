...
// added by Zhen Wu
void dGeomTriMeshGetOriginTriangle(dGeomID g, int Index, dVector3* v0, dVector3* v1, dVector3* v2){
    dUASSERT(g && g->type == dTriMeshClass, "argument not a trimesh");
	...
	const dVector3& pp = *(const dVector3*)&P;
	const dMatrix3& rr = *(const dMatrix3*)&R;

    dVector3 v[3];
	FetchTriangle(Geom, Index, pp, rr, v);
...
void dGeomTriMeshVertexsData (dGeomID g, float* vertexs)
{
	dxTriMesh* Geom = (dxTriMesh *)g;
	const Point *points = Geom->Data->Mesh.GetVerts();
	int vertexs_num = dGeomTriMeshVertexsNum(g);
	for (int i=0; i<vertexs_num; ++i)
	{
		vertexs[3*i] = points[i].x;
		vertexs[3*i+1] = points[i].y;
		vertexs[3*i+2] = points[i].z;
        printf("%f, %f, %f\n", points[i].x, points[i].y, points[i].z);

	}
}

void dGeomTriMeshFacesData (dGeomID g, unsigned int* faces)
{
	dxTriMesh* Geom = (dxTriMesh *)g;
	const IndexedTriangle *indexs = Geom->Data->Mesh.GetTris();
	int faces_num = dGeomTriMeshFacesNum(g);
	for (int i=0; i<faces_num; ++i)
	{
		faces[3*i] = indexs[i].mVRef[0];
		faces[3*i+1] = indexs[i].mVRef[1];
		faces[3*i+2] = indexs[i].mVRef[2];
	}
}

int dGeomTriMeshVertexsNum (dGeomID g)
{
	dxTriMesh* Geom = (dxTriMesh *)g;
	return Geom->Data->Mesh.GetNbVertices();
}

int dGeomTriMeshFacesNum (dGeomID g)
{
	dxTriMesh* Geom = (dxTriMesh *)g;
