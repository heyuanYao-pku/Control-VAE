...
dxGeom::dxGeom (dSpaceID _space, int is_placeable)
{
  ...
  if (_space) 
  {
    // std::cout<<"!"<<parent_space<<std::endl;
...
// // Add by Zhenhua Song
// void dxGeom::append_ignore_geom(dGeomID other) {
//     dAASSERT(ignore_geom_cnt < MAX_GEOM_IGNORE_COUNT);
//     ignore_geom_id[ignore_geom_cnt++] = other;
// }

// // Add by Zhenhua Song
// int dxGeom::is_ignore_geom(dGeomID other) {
//     dAASSERT(ignore_geom_cnt <= MAX_GEOM_IGNORE_COUNT);

//     for (int i = 0; i < ignore_geom_cnt; i++) {
//         if (other == ignore_geom_id[i]) {
//             return 1;
//         }
//     }
...
// add by Yulong Zhang
void dxGeom::computePosr2Copy(dxPosR *tar_posr){
  if (!(gflags & GEOM_POSR_BAD))
    return;
  dMultiply0_331 (tar_posr->pos, body->posr.R, offset_posr->pos);
  tar_posr->pos[0] += body->posr.pos[0];
  tar_posr->pos[1] += body->posr.pos[1];
  tar_posr->pos[2] += body->posr.pos[2];
  dMultiply0_333 (tar_posr->R, body->posr.R, offset_posr->R);
}
...
// add by Yulong Zhang
void dGeomCopyPosRot4Render(dxGeom *g, dxPosR *target_Posr)
{
  for(int i=0;i<4;i++)
    target_Posr->pos[i] = g->final_posr->pos[i];
  for(int i=0;i<12;i++)
    target_Posr->R[i] = g->final_posr->R[i]; 
  g->computePosr2Copy(target_Posr);
}
...
// Add by Zhenhua Song
void dGeomGetQuatAndMatNoNorm(dGeomID geom, dQuaternion quat, dMatrix3 mat)
{
...
// Add by Yulong Zhang
int dGeomGetDrawAxisFlag(dxGeom *g)
{
    dAASSERT(g);
    return g->draw_local_axis;

}

void dGeomSetDrawAxisFlag (dxGeom *g, int x)
{
  dAASSERT(g);
  g->draw_local_axis = x;
}
...
// Add by Zhenhua Song
int dGeomGetCharacterID(dxGeom* g)
{
    dAASSERT (g);
    return g->character_id;
}

// Add by Zhenhua Song
void dGeomSetCharacterID(dxGeom* g, int character_id)
{
    dAASSERT (g);
...
// Add by Zhenhua Song
void dGeomSetIndex(dxGeom* g, int index)
{
    dAASSERT (g);
    g->geom_index = index;
}
...
void dGeomRenderGetUserColor(dxGeom * g, dReal * result)
{
  dAASSERT (g);
  for(int i=0; i<3; i++)
  {
    result[i] = g->render_user_color[i];
  }
}

// Add by Zhenhua Song, for visualize in Long Ge's draw stuff framework
void dGeomRenderInUserColor(dxGeom * g, const dReal * input_arr)
{
  dAASSERT (g);
  for(int i=0; i<3; i++)
  {
    g->render_user_color[i] = input_arr[i];
  }
}

// Add by Zhenhua Song, for visualize in Long Ge's draw stuff framework
void dGeomRenderInDefaultColor(dxGeom * g, int value)
{
  dAASSERT (g);
  g->render_by_default_color = value;
}

// Add by Zhenhua Song, for visualize in Long Ge's draw stuff framework
int dGeomIsRenderInDefaultColor(dxGeom * g)
{
  dAASSERT (g);
  return g->render_by_default_color;
}
