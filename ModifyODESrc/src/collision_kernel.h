...
// Add by Zhenhua Song
#define MAX_GEOM_IGNORE_COUNT 16
...
struct dxGeom : public dBase {
  ...
  int draw_local_axis=0; // whether draw local axis of this geom, add by Yulong Zhang
  ...
  // Add by Zhenhua Song
  //dxGeom* ignore_geom_id[MAX_GEOM_IGNORE_COUNT];
  ...
  // Add by Zhenhua Song, for visualize color in Long Ge's drawstuff framework
  int render_by_default_color = 1;
  dVector3 render_user_color = {0, 0, 0, 0};
  ...
  // Add by Yulong Zhang
  // calc new final position from offset and body and copy to given pointer
  void computePosr2Copy(dxPosR *tar_posr);
