	...
	OBBCache &BoxCache)
{
	...
	if (TriMesh->doBoxTC) {
        ...
        ///////////////////////////////////
        // added by Libin
        // for single-triangle mesh
        if (TriMesh->isSingleTriangle)
            Collider.Collide(*BoxTC, obbCapsule, TriMesh->Data->BVTree, null, null);
        else
        ...
        // for single-triangle mesh
        if (TriMesh->isSingleTriangle)
            Collider.Collide(BoxCache, obbCapsule, TriMesh->Data->BVTree, null, null);
        else
	 ...
	 if (Collider.GetContactStatus()) 
	 {
		 ...
		 if (TriCount != 0)
		 {
			...
			if (cData.m_ctContacts != 0)
			{
                ...
                // added by Libin Liu
                /////////////////////
                if (TriMesh->ColNumCallback)
                {
                    for (unsigned int i = 0; i < cData.m_ctContacts; ++i)
                    {
                        if (cData.m_gLocalContacts[i].nFlags != 0)
                            TriMesh->ColNumCallback(TriMesh, Capsule, cData.m_gLocalContacts[i].triIndex, 1);
                    }
                }
			}
		 }
