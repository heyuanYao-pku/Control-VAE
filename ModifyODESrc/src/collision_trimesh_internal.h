...
/////////////////////////////////
// added by Libin Liu
// we need to know how many contacts are generated for each triangle
// use this callback to inform the caller
typedef void dTriColNumCallback(dGeomID TriMesh, dGeomID RefObject, int TriangleIndex, int nContact);
//////////////////////////////
...
struct dxTriMesh : public dxGeom{
    ...
    // we need to know how many contacts are generated for each triangle
    // use this callback to inform the caller
    dTriColNumCallback *ColNumCallback;

    // for deformable trimesh, we can neglect some computation for efficiency.
    bool isSoftTriMesh;

    // hack we are using two overlapped triangles to avoid restriction of opcode
    // actually we only have to deal with one of them
...
inline void FetchTriangle(dxTriMesh* TriMesh, int Index, const dVector3 Position, const dMatrix3 Rotation, dVector3 Out[3]){
    ...
    ///////////////////////////
    // added by Libin
    // if the trimesh consists of a single triangle, we assume the Position is the origin and Rotation is the identity.
    if (TriMesh->isSingleTriangle)
    {
        for (int i = 0; i < 3; i++)
        {            
