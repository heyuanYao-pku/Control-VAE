...
// draw local axis of a geom, add by Yulong Zhang
DS_API void dsDrawLocalAxis (const float pos[3], const float R[12], const float length);

// add by Yulong Zhang
DS_API void dsTrackBody(dBodyID target, int track_character, int sync_y);

DS_API void dsCameraLookAt(float pos_x, float pos_y, float pos_z, float target_x, float target_y, float target_z, float up_x, float up_y, float up_z);
...
DS_API void dsDrawTriMesh_buffer(const float pos[3], const float R[12],
  const float * vertexs, int vertexs_num, 
  const unsigned int * indexs, int indexs_num);

DS_API void dsDrawTriMesh_(const float pos[3], const float R[12],
  const float * v_pos, int v_num);


DS_API void dsDrawTriMesh(const float pos[3], const float R[12],
  const unsigned char* mesh_list);

DS_API void dsDrawTriMesh1(const float pos[3], const float R[12],
